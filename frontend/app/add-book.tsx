...
export default function AddBookScreen() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      Toast.show({ type: 'error', text1: 'Xatolik', text2: 'Kategoriyalar yuklanmadi' });
    } finally {
  ...
  const pickCoverImage = async () => {
    ...
    if (status !== 'granted') {
      Toast.show({ type: 'error', text1: 'Ruxsat kerak', text2: 'Rasm tanlash uchun ruxsat bering' });
      return;
    }
  ...
  const handleSubmit = async () => {
    if (!title.trim()) return Toast.show({ type: 'error', text1: 'Xatolik', text2: 'Kitob nomini kiriting' });
    ...
    try {
      ...
      formData.append('cover_image', {
        uri: Platform.OS === 'android' ? coverImage.uri : coverImage.uri.replace('file://', ''),
        name: coverImage.name,
        type: coverImage.type,
      } as any);

      formData.append('pdf_file', {
        uri: Platform.OS === 'android' ? pdfFile.uri : pdfFile.uri.replace('file://', ''),
        name: pdfFile.name,
        type: 'application/pdf',
      } as any);
      ...
      const data = await res.json();
      if (!res.ok) {
        throw new Error(Array.isArray(data.message) ? data.message.join(', ') : (data.message || 'Xatolik yuz berdi'));
      }

      router.back();
      setTimeout(() => {
        Toast.show({ type: 'success', text1: 'Muvaffaqiyatli!', text2: `"${title}" kitobi qo'shildi ✅` });
      }, 350);
    } catch (error: any) {
  ...
  const selectedCategoryObjects = categories.filter(c => selectedCategoryIds.includes(c.id));
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: isDark ? '#000' : '#f3f4f6' }}>
      ...
      <View style={{
        ...
        <Text style={{ fontSize: 18, fontWeight: '700', color: isDark ? '#fff' : '#111827' }}>
          Kitob qo'shish
        </Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      >
        ...
        <TouchableOpacity
          ...
          style={{
            ...
            backgroundColor: isDark ? '#1c1c1e' : '#f9fafb',
            borderWidth: 2, borderStyle: 'dashed', borderColor: isDark ? '#444' : '#d1d5db',
            alignItems: 'center', justifyContent: 'center', overflow: 'hidden',
          }}
        >
          {coverImage ? (
            <Image source={{ uri: coverImage.uri }} style={{ width: '100%', height: '100%', borderRadius: 14 }} resizeMode="cover" />
          ) : (
            <View style={{ alignItems: 'center' }}>
              <View style={{
                ...
                <Ionicons name="image-outline" size={28} color={isDark ? '#71717a' : '#9ca3af'} />
              </View>
              <Text style={{ fontSize: 14, fontWeight: '600', color: isDark ? '#71717a' : '#6b7280', marginBottom: 4 }}>
                Rasm tanlang
              </Text>
              <Text style={{ fontSize: 12, color: isDark ? '#52525b' : '#9ca3af' }}>
                JPG, PNG (Max 10MB)
              </Text>
            </View>
          )}
        ...
        <TouchableOpacity
          ...
          style={{
            ...
            borderWidth: 1, borderColor: pdfFile ? '#f97316' : (isDark ? '#333' : '#e5e7eb'),
          }}
        >
          <View style={{
            width: 44, height: 44, borderRadius: 12, marginRight: 14,
            backgroundColor: pdfFile ? '#fff7ed' : (isDark ? '#2c2c2e' : '#f0f0f0'),
            alignItems: 'center', justifyContent: 'center'
          }}>
            <Ionicons
          ...
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 14, fontWeight: '600', color: isDark ? '#fff' : '#111827', marginBottom: 2 }}>
              {pdfFile ? pdfFile.name : 'PDF fayl tanlang'}
            </Text>
            <Text style={{ fontSize: 12, color: isDark ? '#71717a' : '#9ca3af' }}>
        ...
        <TextInput
          style={[inputStyle, { height: 100, textAlignVertical: 'top', paddingTop: 13 }]}
        ...
        <Text style={labelStyle}>
          Kategoriya *{' '}
          <Text style={{ color: selectedCategoryIds.length === MAX_CATEGORIES ? '#ef4444' : '#a1a1aa', fontWeight: '400' }}>
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 10 }}>
            {selectedCategoryObjects.map(cat => (
              ...
              >
                ...
                <TouchableOpacity onPress={() => removeCategory(cat.id)} hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}>
        ...
        <TouchableOpacity
          ...
          style={{
            ...
            borderWidth: 1,
            borderColor: selectedCategoryIds.length > 0 ? '#6366f1' : (isDark ? '#333' : '#e5e7eb'),
          }}
        >
          <Ionicons
            name="layers-outline" size={20}
            color={selectedCategoryIds.length > 0 ? '#6366f1' : (isDark ? '#71717a' : '#9ca3af')}
            style={{ marginRight: 10 }}
          />
          <Text style={{
            flex: 1, fontSize: 15,
            color: selectedCategoryIds.length > 0 ? (isDark ? '#a5b4fc' : '#4f46e5') : (isDark ? '#52525b' : '#9ca3af'),
          }}>
            {selectedCategoryIds.length > 0
              ? `${selectedCategoryIds.length} ta kategoriya tanlandi`
              : 'Kategoriya tanlang'}
          </Text>
          <Ionicons name={showCategoryPicker ? 'chevron-up' : 'chevron-down'} size={20} color={isDark ? '#555' : '#ccc'} />
        </TouchableOpacity>
        ...
        {showCategoryPicker && (
          <View style={{
            ...
            shadowOpacity: isDark ? 0.25 : 0.08, shadowRadius: 12, elevation: 6,
            ...
            {loadingCategories ? (
              ...
              <View style={{ padding: 20, alignItems: 'center' }}>
                <Text style={{ color: isDark ? '#71717a' : '#9ca3af', fontSize: 14 }}>
                  ...
                  return (
                    ...
                    >
                      ...
                      <View style={{
                        ...
                        borderWidth: 2,
                        borderColor: isSelected ? '#6366f1' : (isDark ? '#444' : '#d1d5db'),
        ...
        <TouchableOpacity
          ...
          style={{
            backgroundColor: isSubmitting ? (isDark ? '#374151' : '#d1d5db') : '#f97316',
            ...
            shadowRadius: 12, elevation: isSubmitting ? 0 : 8,
          }}
        >
          {isSubmitting ? (
            <>
              <ActivityIndicator color={isDark ? '#9ca3af' : '#6b7280'} style={{ marginRight: 10 }} />
              <Text style={{ fontSize: 16, fontWeight: '700', color: isDark ? '#9ca3af' : '#6b7280' }}>
                Yuklanmoqda...
              </Text>
            </>
          ) : (
